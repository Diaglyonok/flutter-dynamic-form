...
import 'package:flutter_dynamic_form/model/color_field.dart';

typedef ColorCodeBuilder = Widget Function(BuildContext context, Color color);

class CircleColorPickerController extends ValueNotifier<Color> {
...
class _CircleColorPickerState extends State<CircleColorPicker> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: min(MediaQuery.of(context).size.width - 80, 400),
      height: min(MediaQuery.of(context).size.height - 80, 400),
      child: Stack(
        children: <Widget>[
          ValueListenableBuilder(
              valueListenable:
                  widget.controller ?? ValueNotifier<Color>(const Color.fromARGB(255, 255, 0, 0)),
              builder: (context, value, child) {
                return Center(
                  child: _HuePicker(
                    hue: _hueController.value,
                    size: Size(min(MediaQuery.of(context).size.width - 80, 400),
                        min(MediaQuery.of(context).size.width - 80, 400)),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: LinearGradient(
                  stops: const [0, 0.4, 1],
                  colors: [
                    HSLColor.fromAHSL(1, widget.hue, widget.saturation, 0).toColor(),
                    HSLColor.fromAHSL(1, widget.hue, widget.saturation, 0.5).toColor(),
                    HSLColor.fromAHSL(1, widget.hue, widget.saturation, 0.9).toColor(),
                  ],
                ),
              ),
            ),
...
class _SaturationSliderState extends State<_SaturationSlider> with TickerProviderStateMixin {
  late AnimationController _scaleController;
  Timer? _cancelTimer;
